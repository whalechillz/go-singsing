import { supabase } from "@/lib/supabaseClient";

type TourData = {
  tour: any;
  schedules: any[] | null;
  journeyItems?: any[] | null;
}

export const fetchTourAndSchedulesClient = async (tourId: string): Promise<TourData> => {
  // 투어 정보 가져오기
  const { data: tour, error: tourError } = await supabase
    .from("singsing_tours")
    .select("*")
    .eq("id", tourId)
    .single();
  
  if (tourError) {
    console.error('Tour fetch error:', tourError);
    throw tourError;
  }
  
  // 일정 정보 가져오기
  const { data: schedules, error: schedulesError } = await supabase
    .from("singsing_schedules")
    .select("*")
    .eq("tour_id", tourId)
    .order("date", { ascending: true });
  
  if (schedulesError) {
    console.error('Schedules fetch error:', schedulesError);
  }
  
  // 여정 데이터 가져오기
  const { data: journeyItems, error: journeyError } = await supabase
    .from("tour_journey_items")
    .select("*")
    .eq("tour_id", tourId)
    .order("day_number", { ascending: true })
    .order("order_index", { ascending: true });
  
  if (journeyError) {
    console.error('Journey items fetch error:', journeyError);
  }
  
  // 관계 데이터 추가 조회
  const journeyItemsWithRelations = await Promise.all((journeyItems || []).map(async (item) => {
    let boarding_place = null;
    let spot = null;
    
    if (item.boarding_place_id) {
      const { data, error } = await supabase
        .from("singsing_boarding_places")
        .select("*")
        .eq("id", item.boarding_place_id)
        .single();
      
      if (!error) boarding_place = data;
    }
    
    if (item.spot_id) {
      const { data, error } = await supabase
        .from("tourist_attractions")
        .select("*")
        .eq("id", item.spot_id)
        .single();
      
      if (!error) spot = data;
    }
    
    return { ...item, boarding_place, spot };
  }));
  
  return { 
    tour: tour || null, 
    schedules: schedules || [], 
    journeyItems: journeyItemsWithRelations || [] 
  };
};