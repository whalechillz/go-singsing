      ...
      <tbody>
        {dummyTours.map((tour) => (
          <tr key={tour.id} className="border-t">
            ...
            <td className="py-2 px-4">{tour.dateRange}</td>
            <td className="py-2 px-4">{tour.driverPhone}</td>
            <td className="py-2 px-4 flex gap-2">
              <Link href={`/admin/tours/${tour.id}/edit`} className="text-blue-800 underline hover:text-blue-600 focus:text-blue-600" aria-label="수정">수정</Link>
              <button className="text-red-600 underline hover:text-red-400 focus:text-red-400" aria-label="삭제" disabled>삭제</button>
            </td>
          </tr>
        ))}
      </tbody>
