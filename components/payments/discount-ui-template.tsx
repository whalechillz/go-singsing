// 할인 UI 추가 위치 안내
// PaymentManagerV3.tsx 파일에서 메모(note) 입력 필드를 찾아서 그 다음에 할인 UI를 추가합니다.
  ...
  <div className="mb-3">
    ...
    <select
      ...
      onChange={(e) => {
        ...
        // 할인 유형별 기본 할인명 설정
        if (type === 'coupon') {
          setForm(prev => ({ ...prev, discount_name: '신규가입 쿠폰' }));
        } else if (type === 'event') {
          setForm(prev => ({ ...prev, discount_name: '이벤트 할인' }));
        } else if (type === 'vip') {
          setForm(prev => ({ ...prev, discount_name: 'VIP 할인' }));
        } else if (type === 'special') {
          setForm(prev => ({ ...prev, discount_name: '특별 할인' }));
        }
      }}
    >
