-- singsing_schedules 테이블 구조 개선 (에러 수정 버전)
...
-- 5. boarding_guide_routes 데이터를 schedules로 이동 (수정됨)
-- boarding_guide_routes 데이터 구조 먼저 확인
DO $$
BEGIN
    -- boarding_guide_routes 테이블이 존재하는 경우에만 실행
    IF EXISTS (
        SELECT 1 FROM information_schema.tables 
        WHERE table_name = 'boarding_guide_routes'
    ) THEN
        -- 각 tour_id별로 첫 번째 스케줄에 boarding_info 추가
        ...
        AND ss.id = (
            ...
            WHERE tour_id = bgr.tour_id 
            ORDER BY date 
            LIMIT 1
        )
        AND (ss.boarding_info = '{}'::jsonb OR ss.boarding_info IS NULL);
    END IF;
END $$;
