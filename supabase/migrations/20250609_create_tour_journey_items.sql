...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'tour_journey_items') THEN
    RAISE NOTICE 'tour_journey_items 테이블이 성공적으로 생성되었습니다.';
  END IF;
END $$;
