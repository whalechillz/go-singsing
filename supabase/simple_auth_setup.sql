...
BEGIN
    ...
    -- 2. users 테이블에 추가 (email로 중복 체크)
    -- 관리자
    DELETE FROM users WHERE email = 'admin@singsinggolf.kr' AND id != auth_admin_id::text;
    INSERT INTO users (id, email, name, role, is_active, created_at, updated_at)
    VALUES (auth_admin_id::text, 'admin@singsinggolf.kr', '관리자', 'admin', true, NOW(), NOW())
    ...
    -- 매니저
    DELETE FROM users WHERE email = 'manager@singsinggolf.kr' AND id != auth_manager_id::text;
    INSERT INTO users (id, email, name, role, is_active, created_at, updated_at)
    VALUES (auth_manager_id::text, 'manager@singsinggolf.kr', '매니저', 'manager', true, NOW(), NOW())
    ON CONFLICT (id) DO UPDATE SET 
    ...
    -- 스탭
    DELETE FROM users WHERE email = 'staff@singsinggolf.kr' AND id != auth_staff_id::text;
    INSERT INTO users (id, email, name, role, is_active, created_at, updated_at)
    VALUES (auth_staff_id::text, 'staff@singsinggolf.kr', '스탭', 'staff', true, NOW(), NOW())
    ON CONFLICT (id) DO UPDATE SET 
