...
BEGIN
    ...
    -- 2. users 테이블의 중복 이메일 삭제 (타입 캐스팅 수정)
    DELETE FROM users WHERE email = 'admin@singsinggolf.kr' AND id::text != auth_admin_id::text;
    DELETE FROM users WHERE email = 'manager@singsinggolf.kr' AND id::text != auth_manager_id::text;
    DELETE FROM users WHERE email = 'staff@singsinggolf.kr' AND id::text != auth_staff_id::text;
