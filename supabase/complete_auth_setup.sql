...
BEGIN
    ...
    -- 3. users 테이블에 추가 (phone 필드 포함)
    -- 관리자
    INSERT INTO users (id, name, phone, email, role, is_active, created_at, updated_at)
    VALUES (auth_admin_id, '관리자', '010-1111-1111', 'admin@singsinggolf.kr', 'admin', true, NOW(), NOW())
    ON CONFLICT (id) DO UPDATE SET 
        email = 'admin@singsinggolf.kr',
        name = '관리자',
        phone = COALESCE(users.phone, '010-1111-1111'),
        role = 'admin',
        is_active = true,
        updated_at = NOW();
    ...
    -- 매니저
    INSERT INTO users (id, name, phone, email, role, is_active, created_at, updated_at)
    VALUES (auth_manager_id, '매니저', '010-2222-2222', 'manager@singsinggolf.kr', 'manager', true, NOW(), NOW())
    ON CONFLICT (id) DO UPDATE SET 
        email = 'manager@singsinggolf.kr',
        name = '매니저',
        phone = COALESCE(users.phone, '010-2222-2222'),
        role = 'manager',
        is_active = true,
        updated_at = NOW();
    ...
    -- 스탭
    INSERT INTO users (id, name, phone, email, role, is_active, created_at, updated_at)
    VALUES (auth_staff_id, '스탭', '010-3333-3333', 'staff@singsinggolf.kr', 'staff', true, NOW(), NOW())
    ON CONFLICT (id) DO UPDATE SET 
        email = 'staff@singsinggolf.kr',
        name = '스탭',
        phone = COALESCE(users.phone, '010-3333-3333'),
        role = 'staff',
        is_active = true,
        updated_at = NOW();
